...
#define BST_InterlockedCompareExchangePointer(ptr,_new,val) _InterlockedCompareExchangePointer(ptr,_new,val)
#else
#define BST_InterlockedOr(val,msk)  _InterlockedOr(val,msk)
...
enum : LONG_PTR {
    RTL_SRWLOCK_OWNED_BIT           = 0,
    RTL_SRWLOCK_CONTENDED_BIT       = 1,
    RTL_SRWLOCK_SHARED_BIT          = 2,
    RTL_SRWLOCK_CONTENTION_LOCK_BIT = 3,
    RTL_SRWLOCK_OWNED               = 1 << RTL_SRWLOCK_OWNED_BIT,
    RTL_SRWLOCK_CONTENDED           = 1 << RTL_SRWLOCK_CONTENDED_BIT,
    RTL_SRWLOCK_SHARED              = 1 << RTL_SRWLOCK_SHARED_BIT,
    RTL_SRWLOCK_CONTENTION_LOCK     = 1 << RTL_SRWLOCK_CONTENTION_LOCK_BIT,
    RTL_SRWLOCK_MASK                = RTL_SRWLOCK_OWNED | RTL_SRWLOCK_CONTENDED | RTL_SRWLOCK_SHARED | RTL_SRWLOCK_CONTENTION_LOCK,
    RTL_SRWLOCK_BITS                = 4
};
...
BST_INLINE
void ReleaseWaitBlockLockExclusive(PRTL_SRWLOCK SRWLock, PRTLP_SRWLOCK_WAITBLOCK FirstWaitBlock) noexcept
{
...
BST_INLINE
void ReleaseWaitBlockLockLastShared(PRTL_SRWLOCK SRWLock, PRTLP_SRWLOCK_WAITBLOCK FirstWaitBlock) noexcept
{
    PRTLP_SRWLOCK_WAITBLOCK Next;
    LONG_PTR NewValue;
...
BST_INLINE
void ReleaseWaitBlockLock(IN OUT PRTL_SRWLOCK SRWLock) noexcept
{
...
BST_INLINE
PRTLP_SRWLOCK_WAITBLOCK AcquireWaitBlockLock(PRTL_SRWLOCK SRWLock) noexcept
{
    LONG_PTR PrevValue;
...
BST_INLINE
void AcquireSRWLockSharedWait(PRTL_SRWLOCK SRWLock, PRTLP_SRWLOCK_WAITBLOCK FirstWait, PRTLP_SRWLOCK_SHARED_WAKE WakeChain) noexcept
{
    if (FirstWait != NULL) {
...
BST_INLINE
void InitializeSRWLock(PRTL_SRWLOCK SRWLock) noexcept
{
    SRWLock->Ptr = NULL;
}
...
BST_INLINE
void AcquireSRWLockShared(PRTL_SRWLOCK SRWLock) noexcept
{
...
BST_INLINE 
void ReleaseSRWLockShared(PRTL_SRWLOCK SRWLock) noexcept
{
    LONG_PTR CurrentValue, NewValue;
    PRTLP_SRWLOCK_WAITBLOCK WaitBlock;
    BOOLEAN LastShared;
...
BST_INLINE
void AcquireSRWLockExclusive(PRTL_SRWLOCK SRWLock) noexcept
{
    __declspec(align(16)) RTLP_SRWLOCK_WAITBLOCK StackWaitBlock;
...
BST_INLINE
void ReleaseSRWLockExclusive(PRTL_SRWLOCK SRWLock) noexcept
{
    LONG_PTR CurrentValue, NewValue;
    PRTLP_SRWLOCK_WAITBLOCK WaitBlock;
...
void srw_lock::lock_shared() noexcept
{
    bst::AcquireSRWLockShared(&m_lock);
}

void srw_lock::unlock_shared() noexcept
{
    bst::ReleaseSRWLockShared(&m_lock);
}

void srw_lock::lock_exclusive() noexcept
{
    bst::AcquireSRWLockExclusive(&m_lock);
}

void srw_lock::unlock_exclusive() noexcept
{
    bst::ReleaseSRWLockExclusive(&m_lock);
}

void srw_lock::unlock() noexcept
{
    bst::ReleaseSRWLockExclusive(&m_lock);
    bst::ReleaseSRWLockShared(&m_lock);
}
